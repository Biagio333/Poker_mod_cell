...
class TableSetupActionAndSignals(QObject):
    ...
    def __init__(self, ui):
        ...
        # Nuova logica:
        # screenshot_paths = tutte le sorgenti (path o immagini in memoria serializzate)
        # screenshot_list = SOLO immagine corrente [PIL.Image]
    ...
    # ---------------------------------------------------------------------
    # Helpers nuova logica screenshot
    # ---------------------------------------------------------------------

    def _get_next_screenshot_filename(self, folder):
        folder_path = pathlib.Path(folder)
        files = [
            pathlib.Path.joinpath(folder_path, f)
            for f in os.listdir(folder)
            if os.path.isfile(pathlib.Path.joinpath(folder_path, f))
        ]
        max_index = self.get_max_file_index(files)
        return folder_path / f"screenshot_{max_index + 1:04}.png"


    def _close_current_image(self):
        if self.screenshot_list:
            try:
                self.screenshot_list[0].close()
            except Exception:
                pass
        self.screenshot_list = []

    def _store_in_memory_image(self, image):
        """
        Salva una PIL.Image in bytes PNG.
        Serve per screenshot live, crop e drag&drop senza tenere tutte le immagini aperte in RAM.
        """
        buffer = io.BytesIO()
        image.save(buffer, format="PNG")
        return {"type": "memory", "data": buffer.getvalue()}

    def _open_image_from_source(self, source):
        """
        source può essere:
        - pathlib.Path / str
        - dict {"type":"memory","data":...}
        """
        if isinstance(source, dict) and source.get("type") == "memory":
            return Image.open(io.BytesIO(source["data"]))
        return Image.open(source)

    def load_current_screenshot(self, index=None):
        """
        Carica in self.screenshot_list solo l'immagine corrente.
        """
        ...
        if index is None:
            index = self.selected_screenshot_idx if self.selected_screenshot_idx >= 0 else 0

        if index < 0 or index >= len(self.screenshot_paths):
            return False

        self._close_current_image()

        source = self.screenshot_paths[index]
        img = self._open_image_from_source(source)

        self.screenshot_list = [img]
        self.current_index = index
        self.selected_screenshot_idx = index
        return True

    def get_current_screenshot(self):
        """
        Ritorna la PIL.Image corrente, caricandola se serve.
        """
        if not self.screenshot_paths:
            return None

        desired_index = self.selected_screenshot_idx if self.selected_screenshot_idx >= 0 else 0

        if self.current_index != desired_index or not self.screenshot_list:
            ok = self.load_current_screenshot(desired_index)
            if not ok:
                return None

        return self.screenshot_list[0]

    # ---------------------------------------------------------------------
    # Load / save screenshots
    # ---------------------------------------------------------------------
    ...
    def load_screenshots(self):
        ...
        if folder_str != "":
            try:
                ...
                pngs = sorted([f for f in files if pathlib.Path(f).suffix.lower() == ".png"])

                if self.append_screenshots:
                    self.screenshot_paths.extend(pngs)
                else:
                    self._close_current_image()
                    self.screenshot_paths = pngs

                if self.screenshot_paths:
                    self.selected_screenshot_idx = len(self.screenshot_paths) - 1
                    self.load_current_screenshot(self.selected_screenshot_idx)
                else:
                    self.selected_screenshot_idx = -1
                    self.current_index = -1

                self.cropped = False
                self.update_ui_with_screenshot_amount(len(self.screenshot_paths))
    ...
    def save_screenshots(self):
        ...
        if folder_str != "":
            try:
                ...
                for i, source in enumerate(self.screenshot_paths):
                    out_path = pathlib.Path(folder_str) / f"screenshot_{max_index + 1 + i:04}.png"
                    with self._open_image_from_source(source) as img:
                        img.save(out_path)
    ...
    def get_max_file_index(self, files):
        pngs = [f for f in files if pathlib.Path(f).suffix.lower() == ".png"]
        if len(pngs) == 0:
            return 0
    ...
    def show_screenshot_at_slider_pos(self, value):
        ...
        log.debug("screenshot_paths length: " + str(len(self.screenshot_paths)))

        self.selected_screenshot_idx = value

        if 0 <= self.selected_screenshot_idx < len(self.screenshot_paths):
    ...
    def _connect_cards_with_save_slot(self):
        ...
        save_image_buttons = [
            'call_button', 'raise_button', 'bet_button', 'check_button', 'fold_button',
            'fast_fold_button', 'all_in_call_button', 'my_turn',
    ...
    def _connect_range_buttons_with_save_coordinates(self):
        ...
        range_buttons = ['covered_card_area', 'player_name_area', 'player_funds_area', 'player_pot_area', 'button_search_area']
    ...
    def show_coordinates(self, label, player=None):
        ...
        current_img = self.get_current_screenshot()
        ...
        if player:
            try:
                ...
                log.error(
                    f"Missing table entry for {label} {player}. "
        ...
        self.preview = current_img.crop((x1, y1, x2, y2))
    ...
    def take_screenshot_guard(self):
        if len(self.screenshot_paths) == self.x_times:
            self.timer.stop()
            self.ui.take_screenshot_button.setText("Take screenshot")
            self.is_recording = False
            pop_up("Information", "Screenshots finished")
        elif len(self.screenshot_paths) < self.x_times:
            self.take_screenshot()
            self.ui.take_screenshot_button.setText("Cancel (" + str(self.x_times + 1 - len(self.screenshot_paths)) + ")")
        else:
            self.timer.stop()
            self.is_recording = False
    ...
    def take_screenshot(self):
        ...
        # scegli la cartella la prima volta
        if not self.auto_save_folder:
            folder_str = QtWidgets.QFileDialog.getExistingDirectory(None, 'Select Folder to save screenshots')
            if folder_str == "":
                try:
                    img.close()
                except Exception:
                    pass
                return
            self.auto_save_folder = folder_str

        out_path = self._get_next_screenshot_filename(self.auto_save_folder)
        img.save(out_path)
        ...
        # tengo solo il path
        self.screenshot_paths.append(out_path)
    ...
    def take_screenshot_cropped(self):
        ...
        log.info("Emitting update signal")
        self.preview = self.original_screenshot
        self._update_preview_label(self.preview)
        log.info("signal emission complete")
    ...
    def update_screenshot_pic(self, index):
        ...
        if not self.load_current_screenshot(index):
            return

        log.debug("Convert to pixmap")
        qim = ImageQt(self.screenshot_list[0]).copy()
    ...
    def update_ui_with_screenshot_amount(self, amount):
        ...
        if amount <= 0:
            self.ui.screenshot_slider.setMinimum(-1)
            self.ui.screenshot_slider.setMaximum(-1)
            self.ui.screenshot_slider.setValue(-1)
            return
    ...
    def crop(self):
        ...
        img = self.get_current_screenshot()
        ...
        cropped, found_tlc = crop_screenshot_with_topleft_corner(
            img,
            self.top_left_corner_img,
            False
        )

        if cropped is None or found_tlc is None:
            pop_up(
                "Top left corner problem",
                "No top left corner found in this image."
            )
            return

        self.screenshot_paths[self.selected_screenshot_idx] = self._store_in_memory_image(cropped)
    ...
    def drop_event(self, event):
        if event.mimeData().hasImage():
            ...
            if len(self.screenshot_paths) == 0:
            ...
            self.screenshot_paths.append(pathlib.Path(file_path))
            self.selected_screenshot_idx = len(self.screenshot_paths) - 1
            self.load_current_screenshot(self.selected_screenshot_idx)
    ...
    def get_position(self, event):
        ...
        if self.screenshot_clicks % 2 == 0:
            ...
            if self.x2 > self.x1 and self.y2 > self.y1:
                ...
                self.preview = current_img.crop((self.x1, self.y1, self.x2, self.y2))
                log.info("image cropped")
                self._update_preview_label(self.preview)
        else:
            self.x1 = x
    ...
    def delete(self):
        ...
        owner = mongo.get_table_owner(self.table_name)
        if owner != COMPUTER_NAME:
            pop_up("Not authorized.", "You can only delete the tables that you created")
            return
    ...
    def load(self):
        ...
        all_buttons = [
            ...
            'im_back', 'resume_hand', 'dealer_button', 'covered_card', 'covered_card_area',
            'player_name_area', 'player_funds_area', 'player_pot_area', 'button_search_area',
            'covered_card_area', 'player_name_area', 'player_funds_area', 'player_pot_area',
            'button_search_area', 'left_card_area', 'right_card_area'
        ]
        ...
        current_img = self.get_current_screenshot()
        if current_img is not None and self.top_left_corner_img is not None:
            self.cropped = check_cropping([current_img], self.top_left_corner_img)
        else:
            self.cropped = False
        ...
        for check_box in check_boxes:
            try:
                ...
                elif isinstance(table[check_box], str):
        ...
        try:
            ...
            log.warning("No available data for 'max_players'")
            self.ui.max_players.setCurrentIndex(0)

        exceptions = ["table_name"]
        players_buttons = ['covered_card_area', 'player_name_area', 'player_funds_area', 'player_pot_area', 'button_search_area']
    ...
    def train_model(self):
        ...
        log.info(
            f"You may add additional training images into {TRAIN_FOLDER}. "
            f"Filename does not matter but make sure they are in the correct folder, "
            f"so the network knows the correct label."
        )
        log.info(
            "Note that to speed up training you may want to install cuda and cudnn drivers "
    ...
    def test_all(self):
        ...
        current_img = self.get_current_screenshot()
        if current_img is None:
            pop_up("No screenshot", "Load or take a screenshot first.")
            return

        table_scraper.screenshot = current_img
